...
public static class ClientSettingsDefaults
{
    ...
    public const string AiTagBatchPrompt = """
        请根据下面的多个短剧项目信息，逐个生成：
        1. tags：3-6 个标签，突出题材、关系、情绪或卖点。每个标签不要带 #，程序会自动格式化成 #标签 串联形式。
        2. 标签表达应保持题材化、关系化、情绪化，不要使用低俗、拜金、极端复仇或过度口语化词语。
    ...
    public const string AiFullInfoSystemPrompt =
        ...
        "新剧名、推荐语、标签都必须符合正常宣发尺度，不得包含违背伦理道德、炫富拜金、宣扬极端复仇、色情低俗、羞辱挑衅、擦边暗示等不良价值导向。" +
    ...
    public const string AiFullInfoBatchPrompt = """
        ...
        4. tags：3-6 个标签，突出题材、关系、情绪或卖点。每个标签不要带 #，程序会自动格式化成 #标签 串联形式。
        5. new_title、tagline、tags 都不得出现违背伦理道德、炫富拜金、宣扬极端复仇、色情低俗、过度口语化等不良价值导向。
        6. 文案不要使用“杀疯了、往死里整、陪睡、炫富打脸、赢麻了、家人们谁懂”等风格化措辞。
        7. 输出前必须自行检查：new_title 只能包含 6-15 个汉字，不能多一个字，也不能少一个字；若不满足必须重写，不允许把不合规结果输出。

        输出要求：
        1. 只输出 JSON。
        2. JSON 格式固定为：
        {"items":[{"id":"1","new_title":"...","tagline":"...","synopsis":"...","tags":["...","..."]}]}
        3. 每个输入项目都必须返回一条 items 记录。
        4. 不要遗漏 id。
    ...
    public const string AiFullInfoRetryPrompt = """
        ...
        4. tags 至少 3 个，且不能直接复述完整剧名。
        5. 新剧名、推荐语、标签必须价值导向正常，不得包含炫富拜金、极端复仇、低俗擦边或过度口语化表达。
    ...
    public const string FrameCoverPrompt =
        "请完成以下两个任务：\n" +

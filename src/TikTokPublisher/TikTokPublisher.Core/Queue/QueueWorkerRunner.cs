...
public sealed class QueueWorkerRunner
{
    ...
    public QueueWorkerRunner(UploadSlotCoordinator? sharedUploadSlots = null)
        : this(
            (item, account, log, ct, episodeFallback, runOptions) =>
    ...
    internal QueueWorkerRunner(
        ...
        : this(
            (item, account, log, ct, _, _) => ensureProofMaterial(item, account, log, ct),
            sharedUploadSlots)
    {
    }
        ...
        QueueProofMaterialPrerequisiteWithFallback ensureProofMaterial)
    {
        ...
        if (options.IsAiOutlineSupplementRun())
        {
            uploadLog(
        ...
        // AI outline supplementation edits only the AI-generation material field with an
        // already generated PDF. It must not prepare the account's other configured proof
        // materials here, because that preparation may hydrate missing source videos.
        var aiOutlineSupplement = options.IsAiOutlineSupplementRun();

        if (!aiOutlineSupplement &&
            options.IsStepEnabled(QueueStepRegistry.GenerateTimestampCertificate) &&
            materialTypes.Contains(
                TikTokPublishConstants.FilingOrDistributionLicenseMaterialType,
                StringComparer.Ordinal))
        {
            mutate(() => MarkRunning(item, QueueStepRegistry.GenerateTimestampCertificate));
            Report(
                onProgress,
                workspace,
                item,
                "已勾选备案/发行许可，上传前准备可信时间戳认证证书…",
            ...
            catch (OperationCanceledException) when (ct.IsCancellationRequested)
            {
                mutate(() =>
                {
                    MarkStopped(item, QueueStepRegistry.GenerateTimestampCertificate);
            ...
            {
                var message = $"备案/发行许可时间戳准备失败：{ex.Message}";
                mutate(() =>
                {
                    MarkFailed(item, QueueStepRegistry.GenerateTimestampCertificate, message);
                    item.StepStates[QueueStepRegistry.UploadSeries] = QueueStepStatus.Failed;
                });
                Report(onProgress, workspace, item, message, QueueStepRegistry.GenerateTimestampCertificate);
                return false;
            }
        }

        if (!aiOutlineSupplement &&
            options.IsStepEnabled(QueueStepRegistry.GenerateProofMaterial) &&
            TikTokPublishConstants.RequiresAutoGeneratedCopyrightMaterial(materialTypes))
        {
        ...
        CancellationToken ct)
    {
        ...
        var materialOptions = TikTokMaterialValidationService.Options.FromAccount(
            account,
            settings,
            options,
        ...
        TikTokAccountProfile? account = null)
    {
        ...
        if (stepKey == QueueStepRegistry.MaterialValidate &&
            item.StepStates.GetValueOrDefault(stepKey) == QueueStepStatus.Completed)
        {
            var validationSettings = ClientSettingsStore.Load();
            var validationOptions = TikTokMaterialValidationService.Options.FromAccount(
                account,
                validationSettings,
                options,
                allowMissingUploadVideos: item.IsUploadCompleted);

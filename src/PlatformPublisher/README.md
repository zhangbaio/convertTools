...
- 快手分账个人版：已接入独立经营者管理平台入口、账号级登录态、扫码登录、项目产物解析、第一页表单/封面/承诺函/工程图、单集数量与竖屏封面批量设置、分集视频批量上传、进度和失败检测，以及保持待审核/提交审核终态；默认先保存草稿，配置明确设为 `next` 后才继续视频上传。上传状态保存在 workflow 的 `.kuaishou-personal-upload-state.json`，记录 `miniSeriesId`、当前阶段和失败信息，重试时可直接进入编辑页并跳过已上传视频；`runMode` 支持 `auto/create/edit`，`forceRerun=true` 可忽略旧状态。企业版仍保持独立“待接入”状态，不会错误调用个人版或其他平台。

...
namespace PlatformPublisher.Adx.Security;

public interface IAdxDataProtector
{
    byte[] Protect(byte[] value);
    byte[] Unprotect(byte[] value);
}

public sealed class WindowsAdxDataProtector : IAdxDataProtector
{
    ...
    private static byte[] Transform(byte[] value, bool protect)
    {
        if (!OperatingSystem.IsWindows())
            throw new PlatformNotSupportedException("ADX 凭据安全存储仅支持 Windows。");

...
public static class FinalActionExtensions
{
    ...
    {
        "draft" or "草稿" or "保存草稿" => ChannelsPublisher.Core.Publishing.FinalAction.Draft,
        "publish" or "发表" or "发布" => ChannelsPublisher.Core.Publishing.FinalAction.Publish,
        _ => ChannelsPublisher.Core.Publishing.FinalAction.None,
    };

    public static string ToLabel(this FinalAction action) => action switch
    {
        ChannelsPublisher.Core.Publishing.FinalAction.Draft => "保存草稿",
        ChannelsPublisher.Core.Publishing.FinalAction.Publish => "直接发表",
        _ => "只填不发",
    };
}

...
15. 标题必须是画面的醒目主标题，禁止缩成角落小字；横排时单个汉字高度不得小于整张图片高度的 6%，建议为 7% 到 10%。
16. 竖排时单个汉字宽度不得小于整张图片宽度的 8%，建议为 9% 到 12%，同时保持完整标题不超出画面。
17. 标题超过 6 个字时优先采用两行或三行横排大标题；若沿用原图竖排，必须显著放大，不得一字一行排成细长的小字列。
""";

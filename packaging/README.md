...
- Frida / embeddable Python：打包脚本会下载 Python 3.11 embeddable 和 PyPI 上的 `frida` Windows wheel，放到 `packaging\dependencies\tools\win-x64\python`，供红果高码率「提取启动密钥」开箱使用。不依赖短剧助手，也不要求用户 `pip install frida`。
